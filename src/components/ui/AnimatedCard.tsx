...
export default function AnimatedCard({ title, description, icon: Icon, href, delay = 0 }: AnimatedCardProps) {
  return (
    <Link href={href} className="group block" style={{ animationDelay: `${delay}ms` }}>
      <Card className="h-full flex flex-col bg-card hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:border-accent animate-fadeInUp opacity-0">
